...
class AutoML(BaseEstimator):
    ...
    def __init__(
        ...
        self._max_total_pipelines_num = max_total_pipelines_num
    ...
    def fit(
        ...
        try:
            ...
            with self._stopwatch.time("Run Ensemble Builder"):
                ...
                proc_ensemble = None
                if time_left_for_ensembles <= 0 or (self._max_total_pipelines_num is not None and self.num_run >= self._max_total_pipelines_num):
                    ...
                    if self._ensemble_class is not None:
                        raise ValueError(
                            ...
                            "is no execution budget left. Try increasing the value "
                            "of time_left_for_this_task or max_total_pipelines_num."
                        )
            ...
            with self._stopwatch.time("Run SMAC"):
                ...
                if time_left <= 0 or (self._max_total_pipelines_num is not None and self.num_run >= self._max_total_pipelines_num):
                    self._logger.warning(
                        "Not starting SMAC because there is no execution budget left."
                    )
                    _proc_smac = None
                    ...
                    with self._dask as dask_client:
                        ...
                        _proc_smac = AutoMLSMBO(
                            ...
                            num_run_limit=None if self._max_total_pipelines_num is None else self._max_total_pipelines_num - self.num_run,
    ...
    def fit_pipeline(
        ...
        **kwargs: dict,
        ...
        self.num_run += 1
        if self._max_total_pipelines_num is not None and self.num_run >= self._max_total_pipelines_num:
            raise ValueError(
                "Not fitting a pipeline because there "
                "is no execution budget left. Try increasing the value "
                "of max_total_pipelines_num."
            )
