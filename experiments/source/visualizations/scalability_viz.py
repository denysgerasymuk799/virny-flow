...
def create_performance_plot_folk_emp_all(metrics_df, metric_name: str, base_font_size: int = 22):
    system_order = [VIRNY_FLOW, AUTOSKLEARN, FLAML]
    metric_title = (metric_name.lower().replace('equalized_odds_', '') + 'D').upper() if 'equalized_odds' in metric_name.lower() else metric_name
...
def create_speedup_plot_folk_emp_all(one_worker_metrics_df, final_metrics_df, metric_name: str = 'optimization_time',
                                     dataset: str = 'folk_emp', base_font_size: int = 22):
    # Compute mean runtime per system_name and num_workers
    avg_one_worker_metrics_df = one_worker_metrics_df.groupby(['system_name', 'dataset_name', 'num_workers'])[metric_name].mean().reset_index()
    avg_final_metrics_df = final_metrics_df.groupby(['system_name', 'dataset_name', 'num_workers'])[metric_name].mean().reset_index()

    to_plot = pd.concat([avg_final_metrics_df, avg_one_worker_metrics_df])
    ...
    speedup_askl_workers_32 = to_plot[(to_plot['system_name'] == AUTOSKLEARN) & (to_plot['num_workers'] == 32)]['speedup'].values[0]
    speedup_fl_workers_32 = to_plot[(to_plot['system_name'] == FLAML) & (to_plot['num_workers'] == 32)]['speedup'].values[0]
    best_competitor_workers_32 = speedup_askl_workers_32

    rule_data = pd.DataFrame({
        'speedup': [speedup_askl_workers_32, speedup_fl_workers_32, None],
        'system_name': [AUTOSKLEARN, FLAML, VIRNY_FLOW]
    })
    rules = alt.Chart(rule_data).mark_rule(strokeDash=[8, 4], strokeWidth=3).encode(
        y='speedup',
        color=alt.Color('system_name', title=None, sort=system_order)
    ...
    # Define an arrow to compare virny_flow and ASKL
    speedup_vf_workers_128 = to_plot[(to_plot['system_name'] == VIRNY_FLOW) & (to_plot['num_workers'] == 128)]['speedup'].values[0]
    arrow_head_y_padding = 0.5
    arrow_tail_y_padding = 0.4
    arrow = alt.Chart(pd.DataFrame({
        'x': [128],
        'y_start': [best_competitor_workers_32],
        'y_end': [speedup_vf_workers_128 - arrow_head_y_padding]
    ...
    # Define text to compare virny_flow and FLAML
    comparison_value = round(speedup_vf_workers_128 / best_competitor_workers_32, 2)
    comparison_text = alt.Chart(pd.DataFrame({
        'x': [128],
        'y': [(speedup_vf_workers_128 - 1.5 - best_competitor_workers_32) / 2 + best_competitor_workers_32],
