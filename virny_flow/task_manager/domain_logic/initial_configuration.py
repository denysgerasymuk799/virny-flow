    ...
    for run_num in exp_config.run_nums:
        ...
        while True:
            ...
            if len(logical_pipeline_records) == 0:
                ...
                if await task_queue.is_empty(exp_config_name=exp_config.exp_config_name, run_num=run_num):
                    # Save advisor history for each logical pipeline locally
                    timestamp = datetime.now().strftime('%Y-%m-%d-%H-%M-%S-%f')
                    for lp_name, advisor in lp_to_advisor[run_num].items():
                        advisor["config_advisor"].save_json(filename=f'history/{exp_config.exp_config_name}/run_num_{str(run_num)}/{lp_name}/history_{timestamp}.json')
                    break
...
async def create_init_state_for_config(exp_config: DefaultMunch, db_client: TaskManagerDBClient, run_num: int):
    ...
    exp_config_record = exp_config.toDict()
