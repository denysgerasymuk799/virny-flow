...
class MetricsInteractiveVisualizer:
        ...
        with gr.Blocks(theme=gr.themes.Soft()) as demo:
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    s.change(self._variable_inputs, s, grp_names)
                    s.change(self._variable_inputs, s, grp_dis_values)
    ...
    def _create_model_rank_heatmap(self, model_names: list, tolerance: str, subgroup_accuracy_metrics_lst: list,
                                   subgroup_uncertainty_metrics: list, subgroup_stability_metrics_lst: list,
                                   group_accuracy_metrics_lst: list, group_uncertainty_metrics: list,
                                   group_stability_metrics_lst: list):
        subgroup_model_metrics_matrix, subgroup_sorted_matrix_by_rank = (
            self._create_subgroup_model_rank_heatmap(model_names, subgroup_accuracy_metrics_lst,
                                                     subgroup_uncertainty_metrics, subgroup_stability_metrics_lst, tolerance))
        group_model_metrics_matrix, group_sorted_matrix_by_rank = (
            self._create_group_model_rank_heatmap(model_names, group_accuracy_metrics_lst,
                                                  group_uncertainty_metrics, group_stability_metrics_lst, tolerance))
        model_metrics_matrix = pd.concat([subgroup_model_metrics_matrix, group_model_metrics_matrix])
        sorted_matrix_by_rank = pd.concat([subgroup_sorted_matrix_by_rank, group_sorted_matrix_by_rank])
        model_metrics_matrix = model_metrics_matrix.dropna()
        sorted_matrix_by_rank = sorted_matrix_by_rank.dropna()
    ...
    def _create_subgroup_model_rank_heatmap(self, model_names: list, subgroup_accuracy_metrics_lst: list,
        ...
        return model_metrics_matrix, sorted_matrix_by_rank
    ...
    def _create_metrics_bar_chart_per_one_model(self, model_name: str, metrics_names: list, metrics_type: str):
        ...
        final_chart = (
            alt.layer(
                ...
                width=220 if metrics_type == 'group' else 240,
                height=100
            ).facet(
